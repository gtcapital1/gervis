...
# Script per creare la cartella shared e il file schema.ts
# in un'installazione esistente di Gervis

echo "Inizio setup della cartella 'shared' con lo schema..."

# Crea la cartella shared se non esiste
mkdir -p shared

# Crea il file schema.ts
...
chmod +x shared/schema.ts

echo "Setup completato! La cartella 'shared' con lo schema è stata creata."
echo "Ora puoi eseguire 'npm run db:push' per applicare lo schema al database."